...
use clap::Subcommand;
use jj_lib::{backend::{FileId, TreeValue}, cdc::backend_wrapper::CdcBackendWrapper, file_util::BlockingAsyncReader, local_working_copy::TreeStateSettings, merge::Merge, merged_tree_builder::MergedTreeBuilder, repo::Repo, repo_path::RepoPathBuf, store::Store};
use tracing::instrument;

use crate::{cli_util::{CommandHelper}, command_error::{CommandError, user_error}, ui::Ui};


#[derive(Subcommand, Clone, Debug)]
pub(crate) enum CdcCommand {
    /// Add files to the CDC set, converting them from git storage to CDC storage
    Add(CdcSetArgs),
    /// Remove files from the CDC set, converting them from CDC storage back to git storage
    Remove(CdcSetArgs),
}
...
async fn convert_file_storage(
    ...
    let new_id = if to_cdc {
        ...
        let pointer_content = cdc_backend.write_file_to_cdc(file).await.map_err(|e|{
            user_error(e)
        })?;
        store.write_file(path, &mut std::io::Cursor::new(pointer_content)).await?
...
async fn cmd_cdc_set(
    ...
    let wc_commit_id = tx.base_workspace_helper()
        .get_wc_commit_id()
    ...
    for (path, value_result) in wc_tree.entries_matching(&fileset.to_matcher()) { 
        ...
        let Some(Some(TreeValue::File { id, executable, copy_id })) = value.as_resolved()
        else {
            writeln!(
                ui.warning_default(),
        ...
        let new_id = convert_file_storage(&path, ws_path, store, &tree_state_settings, to_cdc).await?;
    ...
    if to_cdc {
        ...
        tx.finish(ui, format!("cdc add: {}", args.paths.join(" "))).await?;
    } 
    else {
        writeln!(ui.status(), "{} files changed from cdc to git", changed)?;
        tx.finish(ui, format!("cdc remove: {}", args.paths.join(" "))).await?;
    }
    Ok(())
}
