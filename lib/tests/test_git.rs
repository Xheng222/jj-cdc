...
fn test_import_refs() {
    ...
    git::import_head(tx.repo_mut(), jj_lib::ref_name::WorkspaceName::DEFAULT).block_on().unwrap();
...
fn test_import_refs_missing_git_commit() {
    ...
    let mut tx = repo.start_transaction();
    let result = git::import_head(tx.repo_mut(), jj_lib::ref_name::WorkspaceName::DEFAULT).block_on();
    assert_matches!(
        result,
...
fn test_export_refs_no_detach() {
    ...
    git::import_head(mut_repo, jj_lib::ref_name::WorkspaceName::DEFAULT).block_on().unwrap();
